...
import GlobalStyleheet from '../../../constants/Styles';
...
const Register = () => {
  ...
  const GlobalStyle = GlobalStyleheet({wp, hp});
  ...
  const ValidateValidMailOrNum = async value => {
    ...
    if (REGISTER_TYPE_EMAIL) {
      ...
      if (isValidEmail) {
        try {
          const response = await FindUserByEmailApi(CONFIG.APP_ID, value);
          if (response) {
      ...
      if (isValidPhoneNumber && String(value).length === 10) {
        try {
          const response = await FindUserByPhoneNumberApi(CONFIG.APP_ID, value);
